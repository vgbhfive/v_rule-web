...
import type { InterestInfo, InterestParams } from '#/api/product';
...
// 表单提交处理
async function onSubmit(values: InterestParams) {
  await gridApi.reload({ ...values });
}

// 表格配置
const gridOptions: VxeGridProps<InterestInfo> = {
  columns: [
    ...
    },
    {
      ...
      slots: {
        default: ({ row }: { row: InterestInfo }) =>
          unitMap.value[row.unit] || row.unit,
      },
    },
  ...
  proxyConfig: {
    ajax: {
      query: async ({ page }, form) => {
        ...
        return await getInterestList(params);
      },
    },
  },
  sortConfig: {
...
// 详情
async function handleInfo(row: InterestInfo) {
  drawerTitle.value = '利率产品详情';
