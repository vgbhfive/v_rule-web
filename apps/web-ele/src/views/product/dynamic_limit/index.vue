...
import type { DynamicLimitInfo, DynamicLimitParams } from '#/api/product';
...
const [QueryForm, queryFormApi] = useVbenForm({
  ...
  submitButtonOptions: {
    content: '查询',
    show: hasAccessByCodes(['product_dynamic_limit_manage']),
  },
  wrapperClass: 'grid-cols-3 grid-cols-4',
});

// 表单提交处理
async function onSubmit(values: DynamicLimitParams) {
  await gridApi.reload({ ...values });
}

// 表格配置
const gridOptions: VxeGridProps<DynamicLimitInfo> = {
  columns: [
    { type: 'seq', title: '序号', width: 50 },
    {
      ...
      title: '业务线',
      slots: {
        default: ({ row }: { row: DynamicLimitInfo }) =>
    ...
    { field: 'productNo', title: '编码' },
    {
      ...
      slots: {
        default: ({ row }: { row: DynamicLimitInfo }) =>
          dataSourceMap.value[row.startLimit] || row.startLimit,
      },
    },
    {
      field: 'endLimit',
      ...
      slots: {
        default: ({ row }: { row: DynamicLimitInfo }) =>
          dataSourceMap.value[row.endLimit] || row.endLimit,
      },
    },
    {
      field: 'limitStep',
      ...
      slots: {
        default: ({ row }: { row: DynamicLimitInfo }) =>
          dataSourceMap.value[row.limitStep] || row.limitStep,
      },
    },
    ...
    { field: 'createAt', title: '创建时间' },
    {
      ...
      width: 200,
      slots: {
        default: ({ row: dynamicLimitInfo }: { row: DynamicLimitInfo }) => {
          const buttons = [];

          if (hasAccessByCodes(['product_dynamic_limit_manage_update'])) {
            buttons.push(
              h(
                ElButton,
                {
                  type: 'primary',
                  size: 'small',
                  onClick: () => handleEdit(dynamicLimitInfo),
                },
                { default: () => '编辑' },
              ),
            );
          ...
          buttons.push(
            h(
              ElButton,
              {
                type: 'info',
                size: 'small',
                onClick: () => handleInfo(dynamicLimitInfo),
              },
              { default: () => '详情' },
            ),
          );

          if (hasAccessByCodes(['product_dynamic_limit_manage_valid'])) {
            if (dynamicLimitInfo.isValid === 0) {
              buttons.push(
                h(
                  ElButton,
                  {
                    type: 'success',
                    size: 'small',
                    onClick: () => handleValid(dynamicLimitInfo),
                  },
                  { default: () => '生效' },
                ),
              );
              ...
              buttons.push(
                h(
                  ElButton,
                  {
                    type: 'warning',
                    size: 'small',
                    onClick: () => handleInvalid(dynamicLimitInfo),
                  },
                  { default: () => '失效' },
                ),
              );
  ...
  proxyConfig: {
    ajax: {
      query: async ({ page }, form) => {
        ...
        return await getDynamicLimitList(params);
      },
    },
  },
  sortConfig: {
...
const gridEvents: VxeGridListeners<DynamicLimitInfo> = {
  // 可以添加表格事件监听
};
...
const currentEditing = ref<DynamicLimitInfo | null>(null);
...
// 详情
async function handleInfo(row: DynamicLimitInfo) {
  drawerTitle.value = '动态额度产品详情';
  ...
  const values = {
    ...row,
    startValueType: row.startLimitType,
    startValueFixed: row.startLimit,
    startValueDataSource: row.startLimit,
    endValueType: row.endLimitType,
    endValueFixed: row.endLimit,
    endValueDataSource: row.endLimit,
    stepValueType: row.limitStepType,
    stepValueFixed: row.limitStep,
    stepValueDataSource: row.limitStep,
  };
  if (
    values.startLimitType === 'dataSource' ||
    values.endLimitType === 'dataSource' ||
    values.limitStepType === 'dataSource'
  ) {
    const data = {
  ...
  editFormApi.updateSchema([
    ...
    },
    {
      ...
      hide: !(row.startLimitType === 'dataSource'),
    },
    {
      fieldName: 'endValueType',
      componentProps: {
    ...
    },
    {
      ...
      hide: !(row.endLimitType === 'dataSource'),
    },
    ...
    },
    {
      ...
      hide: !(row.limitStepType === 'dataSource'),
    },
    {
      fieldName: 'remark',
      componentProps: {
...
// 编辑
async function handleEdit(row: DynamicLimitInfo) {
  drawerTitle.value = '编辑动态额度产品';
  currentEditing.value = row;
  // 重置表单以清除之前的校验状态
  editFormApi.resetForm();
  // 填充表单数据
  ...
  isAdd.value = false;
  isEdit.value = true;
  preStartValueType.value = row.startLimitType;
  preEndValueType.value = row.endLimitType;
  preStepValueType.value = row.limitStepType;
  editDrawerVisible.value = true;
}

// 生效
async function handleValid(row: DynamicLimitInfo) {
  try {
    const data = { id: row.id, isValid: 1 };
    const resp = await updateDynamicLimitValid(data);
...
// 失效
async function handleInvalid(row: DynamicLimitInfo) {
  try {
  ...
  try {
    if (isAdd.value) {
      const insertData = {
        ...
        type: 'dynamic_limit',
        startLimitType: values.startValueType,
        startLimit:
          values.startValueType === 'fixed'
            ? values.startValueFixed
            : values.startValueDataSource,
        endLimitType: values.endValueType,
        endLimit:
          values.endValueType === 'fixed'
            ? values.endValueFixed
            : values.endValueDataSource,
        limitStepType: values.stepValueType,
        limitStep:
          values.stepValueType === 'fixed'
      ...
      const resp = await createDynamicLimit(insertData);
      ElMessage.success(resp);
    }
    ...
    if (isEdit.value) {
      ...
      const resp = await updateDynamicLimit(updateData);
      ElMessage.success(resp);
    }
    ...
    ElMessage.error('动态额度操作失败');
    console.error('动态额度操作失败:', error);
  }
}
...
async function handleResetEdit() {
  ...
  if (isEdit.value) {
    const data = {
      ...
      endLimitFixed: currentEditing.value?.endLimit,
      endLimitDataSource: currentEditing.value?.endLimit,
      limitStepFixed: currentEditing.value?.limitStep,
      limitStepDataSource: currentEditing.value?.limitStep,
    };
  ...
  <Page description="产品动态额度管理">
    <div>
      ...
      <div
        ...
        v-if="hasAccessByCodes(['product_dynamic_limit_manage_create'])"
      >
